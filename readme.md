...
25. 이미 가입된 사용자의 가입요청 처리

이미 가입이된 상태의 사용자가 동일한 계정으로 가입을 요청하는 경우가 있습니다. 재등록을 피하기 위해 기존에 등록된 사용자인지 확인하는 과정을 추가합니다.

- controllers/userController.js : postJoin에 사용자가 보내온 email을 db에서 검색하는 과정을 추가합니다. 이미 등록된 경우에는 error를 보냅니다.

26. 사용자의 비밀번호 암호화

사용자의 비밀번호를 암호화 하지 않은 상태로 저장하는 것은 매우 위험하기 때문에 암호화 후에 저장을 하려합니다. 또한 로그인시에는 비밀번호를 암호화된 것과 대조하고, 로그아웃시에는 세션을 없애도록 수정하겠습니다. 암호화에 사용하려는 모듈은 bcrypt 입니다.
...
- /controller/userController.js :
  1. postJoin 메서드에서 Password를 그대로 저장하고 있습니다. 암호화 모듈을 이용하여 암호화 한 비밀번호를 데이터 베이스에 저장하겠습니다.
  2. getLogout 메서드에 req.session.destroy()를 이용하여 로그인시에는 세션을 없애도록 했습니다.
- passport.js : 비밀번호 확인시 평문의 비밀번호를 받아와 암호화된 비밀번호와 비교해야합니다. 기존의 비밀번호 확인절차를 bcrypt에서 제공하는 compare메서드를 이용하는 것으로 바꾸었습니다.
